...
//               |      root.tabs.subtabs
//               |      PARALLEL          \
//               |      stateParam   root.tabs.subtabs.S2
//               |                            PARALLEL
//               |
...
// root.tabs.deep: This parallel state has deeply nested states and has stateParams.
//               This demonstrates parallel states being re-activated with different state params.
//               This also demonstrates the tab_controls, where clicking the "deep nest" tab will bring you
//               back to whatever deeply nested state you last activated within the root.tabs.deep subtree.
...
angular.module('parallel').config([ '$stateProvider', '$urlRouterProvider', function ($stateProvider, $urlRouterProvider) {
  ...
  $stateProvider.state('root.tabs', {
    ...
    template: '<b>root.tabs</b>: Started {{delta}} seconds ago' +
            ...
            '  <div ng-show="showInactiveTabs || $state.includes(\'root.tabs.people\')"><div ui-view="people">Nothing Loaded</div></div>' +
            '  <div ng-show="showInactiveTabs || $state.includes(\'root.tabs.subtabs\')"><div ui-view="subtabs">Nothing Loaded</div></div>' +
            '  <div ng-show="showInactiveTabs || $state.includes(\'root.tabs.deep\')"><div ui-view="S2">Nothing Loaded</div></div>' +
            // the state root.tabs.outersubtags is NOT a parallel state.
            '  <div ng-show="showInactiveTabs || $state.includes(\'root.tabs.outersubtabs\')"><div ui-view="outersubtabs">Nothing Loaded</div></div>' +
            '',
    url: 'tabs'
  });
  ...
  $stateProvider.state('root.tabs.people', {
    ...
    resolve: {
      people: function() {
        console.log("resolve: people (root.tabs.people)");
        return [
  ...
  $stateProvider.state('root.tabs.deep', {
    views: {
      S2: {
        controller:  function ($scope, $state, $timeout, $stateParams) {
          if (!$stateParams.id) {
            $state.go("root.tabs.deep", { id: 100}); // No id parameter.  Redirect with a id parameter
          } else {
            $scope.id = $stateParams.id;
            timerCtrl($scope, $state, $timeout);
          }
        },
        template: '<b>root.tabs.deep (id: {{id}})</b>: Started {{delta}} seconds ago' +
                '<br><input ng-model="data" type="text"> {{data}}' +
...
//                '<br>Not Parallel...' +
                ...
                '<br><input type="checkbox" ng-model="showInactiveTabs">Show inactive tabs' +
                '<ul class="tabs" parallel-state-controls>' +
                '   <li ng-class="{ active: $state.includes(\'root.tabs.subtabs.S1\') }" parallel-state-selector=".S1">subtabs.S1 (Nested Parallel)</li>' +
                '   <li ng-class="{ active: $state.includes(\'root.tabs.subtabs.S2\') }" parallel-state-selector=".S2">subtabs.S2 (Nested Parallel)</span>' +
                '</ul>' +
                // Here is where the parallel states are bound to the UI.
                // Note: Wrap the ui-view in a div because ng-show doesn't seem to work on a ui-view
                '  <div ng-show="showInactiveTabs || $state.includes(\'root.tabs.subtabs.S1\')"><div id="root_tabs_ui-view_S1" ui-view="S1">Nothing Loaded</div></div>' +
                '  <div ng-show="showInactiveTabs || $state.includes(\'root.tabs.subtabs.S2\')"><div id="root_tabs_ui-view_S2" ui-view="S2">Nothing Loaded</div></div>' +
                ''
      }
    },
    parallel: true,
    ...
    views: {
      outersubtabs: {
        ...
        template:
                ...
                '<br><b>root.tabs.outersubtabs</b>: Started {{delta}} seconds ago' +
                '<br><input ng-model="data" type="text">{{data}}' +
                '<br><input type="checkbox" ng-model="showInactiveTabs">Show inactive tabs' +
                '<ul class="tabs" parallel-state-controls>' +
                '   <li ng-class="{ active: $state.includes(\'root.tabs.outersubtabs.S1\') }" parallel-state-selector=".S1">outersubtabs.S1 (Outer Parallel State 1)</li>' +
                '   <li ng-class="{ active: $state.includes(\'root.tabs.outersubtabs.S2\') }" parallel-state-selector=".S2">outersubtabs.S2 (Outer Parallel State 2)</li>' +
                '</ul>' +
                // Here is where the parallel states are bound to the UI.
                // Note: Wrap the ui-view in a div because ng-show doesn't seem to work on a ui-view
                '  <div ng-show="showInactiveTabs || $state.includes(\'root.tabs.outersubtabs.S1\')"><div id="root_tabs_ui-view_S1" ui-view="S1">Nothing Loaded</div></div>' +
                '  <div ng-show="showInactiveTabs || $state.includes(\'root.tabs.outersubtabs.S2\')"><div id="root_tabs_ui-view_S2" ui-view="S2">Nothing Loaded</div></div>' +
                ''
      }
    },
    url: '/outersubtabs'
  ...
  // Deeply nested state of root.tabs.deep
  $stateProvider.state('root.tabs.deep.deep', {
    controller: timerCtrl,
    template: '<b>root.tabs.deep.deep</b>: Started {{delta}} seconds ago' +
  ...
  // Deeply nested state of root.tabs.deep
  $stateProvider.state('root.tabs.deep.deep.nest', {
    controller: timerCtrl,
    template: '<b>root.tabs.deep.deep.nest</b>: Started {{delta}} seconds ago' +
            '<input ng-model="data" type="text">{{data}}' +
            '<br><a ui-sref="root.tabs.deep.deep.nest({id: 10})">Change root.tabs.deep id param to 10</a>' +
            '<br><a ui-sref="root.tabs.deep.deep.nest({id: 100})">Change root.tabs.deep id param to 100</a>' +
            '<br><a ui-sref="root.tabs.deep.deep.nest({id: 9127})">Change root.tabs.deep id param to 9127</a>' +
            '',
    url: '/nest'
  });
}]);
